...
#
# First-class Python compute script (promoted from 06_prisma_diagram.sh —
# bash wrapper only did mkdir + a python heredoc; stdlib only).
import csv
import os
...
svg_parts.append(text(cx, y_id + box_h // 2, ["Records identified", f"PubMed (n={n_id})"], size=11, weight="bold"))

svg_parts.append(arrow(cx, y_id + box_h, cx, y_ret))

svg_parts.append(box(bx, y_ret, box_w, box_h, "#cce5ff", "#0066cc"))
svg_parts.append(text(cx, y_ret + box_h // 2, ["Records retrieved", f"(n={n_ret})"], size=11))

# Duplicate exclusion side box
svg_parts.append(side_box(cx + box_w // 2 + 10, y_ret, 170, box_h,
                          ["Duplicates removed", f"(n={n_dup})"], "#ffe0cc", "#cc6600"))
...
svg_parts.append(text(cx, y_scr + box_h // 2, ["Records screened", f"(n={n_scr})"], size=11))

# Excluded side box
svg_parts.append(side_box(cx + box_w // 2 + 10, y_scr, 170, box_h,
                          ["Excluded (abstract)", f"(n={n_exc_abs})"], "#f8d7da", "#721c24"))
...
svg_parts.append(text(cx, y_unc + box_h // 2, ["Full-text assessed", f"(n={n_unc})"], size=11))

# Excluded at full text
svg_parts.append(side_box(cx + box_w // 2 + 10, y_unc, 170, box_h,
                          ["Excluded (full-text)", f"(n={n_re})"], "#f8d7da", "#721c24"))
...
svg_parts.append(text(cx, y_inc + box_h // 2, ["Included (abstract screen)", f"(n={n_inc_abs})"], size=11))

svg_parts.append(arrow(cx, y_inc + box_h, cx, y_tot))

# Total
svg_parts.append(box(bx, y_tot, box_w, box_h - 10, "#155724", "#0d3b1e"))
svg_parts.append(text(cx, y_tot + (box_h - 10) // 2, ["Total included", f"n = {n_total}"], size=13, weight="bold", fill="#ffffff"))
...
print("prisma_diagram done", flush=True)
